...
```{r, include = FALSE, message = FALSE}
#Reading in data
abortion_s1 <- read_csv("Abortion Study_December 4, 2023_21.29.csv")
```

```{r, include = FALSE, message = FALSE}
#Filtering abortion dataset
...
```{r, include = FALSE, message = FALSE}
#Creating Study 1 graph for effects of misinformation + fact-checks on factual beliefs
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(fit1_s1b, include.ci = FALSE,
       ...
       stars = c(0.05, 0.01, 0.001), 
       caption = "Interaction between frequently/never (vs. sometimes) encountering fact checks and fact
check of claim that abortion pills are dangerous (1-to-4 scale)",
       caption.above = TRUE, float.pos = "H",
       single.row = TRUE)
```
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(fit2_s1b, include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact check", "Frequently encounter fact checks", "Never encounter fact checks",
        "Control*Frequently encounter fact checks", "Fact-check*Frequently encounter fact checks", "Control*Never encounter fact checks", "Fact-check*Never encounter fact checks"),
       stars = c(0.05, 0.01, 0.001), 
       caption = "Interaction between frequently/never (vs. sometimes) encountering fact checks and fact
check of claim that more contraception availability increases abortion demand (1-to-4 scale)",
       caption.above = TRUE, float.pos = "H",
       single.row = TRUE)
```
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(fit3_s1b, include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact check", "Frequently encounter fact checks", "Never encounter fact checks",
        "Control*Frequently encounter fact checks", "Fact-check*Frequently encounter fact checks", "Control*Never encounter fact checks", "Fact-check*Never encounter fact checks"),
       stars = c(0.05, 0.01, 0.001), 
       caption = "Interaction between frequently/never (vs. sometimes) encountering fact checks and fact
check of claim about 10-year-old terminating pregnancy (1-to-4 scale)",
       caption.above = TRUE, float.pos = "H",
       single.row = TRUE)
```
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(fit1_s1c, include.ci = FALSE,
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(fit2_s1c, include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact-check", "Independent/Other", "Republican", 
      "Control*Independent/Other", "Fact-check*Independent/Other", "Control*Republican", "Fact-check*Republican"),
       stars = c(0.05, 0.01, 0.001), 
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(fit3_s1c, include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact-check", "Independent/Other", "Republican", 
      "Control*Independent/Other", "Fact-check*Independent/Other", "Control*Republican", "Fact-check*Republican"),
      stars = c(0.05, 0.01, 0.001), 
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(fit1_s1d, include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact-check",
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(fit2_s1d, include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact-check",
      "High political knowledge", "Low political knowledge",
      "Control*High political knowledge", "Fact-check*High political knowledge",
      "Control*Low political knowledge", "Fact-check*Low political knowledge"),
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(fit3_s1d, include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact-check",
      "High political knowledge", "Low political knowledge",
      "Control*High political knowledge", "Fact-check*High political knowledge",
      "Control*Low political knowledge", "Fact-check*Low political knowledge"),
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(fit1_s1e, include.ci = FALSE,
...
```{r, include = FALSE, warning = FALSE}
#Comparing the effect of the 1st fact check for the high and low religiosity groups
pred_fit1_s1e_misinfo_high <-
  as.numeric(
  fit1_s1e$coefficients["(Intercept)"] + 
  fit1_s1e$coefficients["Z_fc_1control"]*0 +
  fit1_s1e$coefficients["Z_fc_1factcheck"]*0 +
  fit1_s1e$coefficients["attend_cleanedHigh"]*1 +
  fit1_s1e$coefficients["attend_cleanedLow"]*0 +
  fit1_s1e$coefficients["Z_fc_1control:attend_cleanedHigh"]*0 +
  fit1_s1e$coefficients["Z_fc_1factcheck:attend_cleanedHigh"]*0 +
  fit1_s1e$coefficients["Z_fc_1control:attend_cleanedLow"]*0 +
  fit1_s1e$coefficients["Z_fc_1factcheck:attend_cleanedLow"]*0
)
pred_fit1_s1e_misinfo_low <-
  as.numeric(
  fit1_s1e$coefficients["(Intercept)"] + 
  fit1_s1e$coefficients["Z_fc_1control"]*0 +
  fit1_s1e$coefficients["Z_fc_1factcheck"]*0 +
  fit1_s1e$coefficients["attend_cleanedHigh"]*0 +
  fit1_s1e$coefficients["attend_cleanedLow"]*1 +
  fit1_s1e$coefficients["Z_fc_1control:attend_cleanedHigh"]*0 +
  fit1_s1e$coefficients["Z_fc_1factcheck:attend_cleanedHigh"]*0 +
  fit1_s1e$coefficients["Z_fc_1control:attend_cleanedLow"]*0 +
  fit1_s1e$coefficients["Z_fc_1factcheck:attend_cleanedLow"]*0
)
pred_fit1_s1e_fc_high <-
  as.numeric(
  fit1_s1e$coefficients["(Intercept)"] + 
  fit1_s1e$coefficients["Z_fc_1control"]*0 +
  fit1_s1e$coefficients["Z_fc_1factcheck"]*1 +
  fit1_s1e$coefficients["attend_cleanedHigh"]*1 +
  fit1_s1e$coefficients["attend_cleanedLow"]*0 +
  fit1_s1e$coefficients["Z_fc_1control:attend_cleanedHigh"]*0 +
  fit1_s1e$coefficients["Z_fc_1factcheck:attend_cleanedHigh"]*1 +
  fit1_s1e$coefficients["Z_fc_1control:attend_cleanedLow"]*0 +
  fit1_s1e$coefficients["Z_fc_1factcheck:attend_cleanedLow"]*0
)
pred_fit1_s1e_fc_low <-
  as.numeric(
  fit1_s1e$coefficients["(Intercept)"] + 
  fit1_s1e$coefficients["Z_fc_1control"]*0 +
  fit1_s1e$coefficients["Z_fc_1factcheck"]*1 +
  ...
  fit1_s1e$coefficients["Z_fc_1factcheck:attend_cleanedHigh"]*0 +
  fit1_s1e$coefficients["Z_fc_1control:attend_cleanedLow"]*0 +
  fit1_s1e$coefficients["Z_fc_1factcheck:attend_cleanedLow"]*1
)

pred_fit1_s1e_fc_high-pred_fit1_s1e_misinfo_high
pred_fit1_s1e_fc_low-pred_fit1_s1e_misinfo_low
```
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(fit2_s1e, include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact check", "High attendance", "Low attendance",
        "Control*High attendance", "Fact check*High attendance", "Control*Low attendance", "Fact check*Low attendance"),
       stars = c(0.05, 0.01, 0.001), 
...
```{r, include = FALSE, warning = FALSE}
#Comparing the effect of the 2nd fact check for the high and low religiosity groups
pred_fit2_s1e_misinfo_high <-
  as.numeric(
  fit2_s1e$coefficients["(Intercept)"] + 
  fit2_s1e$coefficients["Z_fc_2control"]*0 +
  fit2_s1e$coefficients["Z_fc_2factcheck"]*0 +
  fit2_s1e$coefficients["attend_cleanedHigh"]*1 +
  fit2_s1e$coefficients["attend_cleanedLow"]*0 +
  fit2_s1e$coefficients["Z_fc_2control:attend_cleanedHigh"]*0 +
  fit2_s1e$coefficients["Z_fc_2factcheck:attend_cleanedHigh"]*0 +
  fit2_s1e$coefficients["Z_fc_2control:attend_cleanedLow"]*0 +
  fit2_s1e$coefficients["Z_fc_2factcheck:attend_cleanedLow"]*0
)
pred_fit2_s1e_misinfo_low <-
  as.numeric(
  fit2_s1e$coefficients["(Intercept)"] + 
  fit2_s1e$coefficients["Z_fc_2control"]*0 +
  fit2_s1e$coefficients["Z_fc_2factcheck"]*0 +
  fit2_s1e$coefficients["attend_cleanedHigh"]*0 +
  fit2_s1e$coefficients["attend_cleanedLow"]*1 +
  fit2_s1e$coefficients["Z_fc_2control:attend_cleanedHigh"]*0 +
  fit2_s1e$coefficients["Z_fc_2factcheck:attend_cleanedHigh"]*0 +
  fit2_s1e$coefficients["Z_fc_2control:attend_cleanedLow"]*0 +
  fit2_s1e$coefficients["Z_fc_2factcheck:attend_cleanedLow"]*0
)
pred_fit2_s1e_fc_high <-
  as.numeric(
  fit2_s1e$coefficients["(Intercept)"] + 
  fit2_s1e$coefficients["Z_fc_2control"]*0 +
  fit2_s1e$coefficients["Z_fc_2factcheck"]*1 +
  fit2_s1e$coefficients["attend_cleanedHigh"]*1 +
  fit2_s1e$coefficients["attend_cleanedLow"]*0 +
  fit2_s1e$coefficients["Z_fc_2control:attend_cleanedHigh"]*0 +
  fit2_s1e$coefficients["Z_fc_2factcheck:attend_cleanedHigh"]*1 +
  fit2_s1e$coefficients["Z_fc_2control:attend_cleanedLow"]*0 +
  fit2_s1e$coefficients["Z_fc_2factcheck:attend_cleanedLow"]*0
)
pred_fit2_s1e_fc_low <-
  as.numeric(
  fit2_s1e$coefficients["(Intercept)"] + 
  fit2_s1e$coefficients["Z_fc_2control"]*0 +
  fit2_s1e$coefficients["Z_fc_2factcheck"]*1 +
  ...
  fit2_s1e$coefficients["Z_fc_2factcheck:attend_cleanedHigh"]*0 +
  fit2_s1e$coefficients["Z_fc_2control:attend_cleanedLow"]*0 +
  fit2_s1e$coefficients["Z_fc_2factcheck:attend_cleanedLow"]*1
)

pred_fit2_s1e_fc_high-pred_fit2_s1e_misinfo_high
pred_fit2_s1e_fc_low-pred_fit2_s1e_misinfo_low
```
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(fit3_s1e, include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact check", "High attendance", "Low attendance",
        "Control*High attendance", "Fact check*High attendance", "Control*Low attendance", "Fact check*Low attendance"),
       stars = c(0.05, 0.01, 0.001), 
...
```{r, echo = FALSE, results='asis'}
#Running models
fit4_s1_dem <- lm_robust(thermometer_1 ~ Z_fc_1, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$pid_1_cleaned == "Democrat",])
fit4_s1_rep <- lm_robust(thermometer_1 ~ Z_fc_1, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$pid_1_cleaned == "Republican",])
fit4_s1_other <- lm_robust(thermometer_1 ~ Z_fc_1, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$pid_1_cleaned == "Independent/Other",])

fit5_s1_dem <- lm_robust(thermometer_2 ~ Z_fc_2, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$pid_1_cleaned == "Democrat",])
fit5_s1_rep <- lm_robust(thermometer_2 ~ Z_fc_2, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$pid_1_cleaned == "Republican",])
fit5_s1_other <- lm_robust(thermometer_2 ~ Z_fc_2, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$pid_1_cleaned == "Independent/Other",])

fit6_s1_dem <- lm_robust(thermometer_3 ~ Z_fc_3, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$pid_1_cleaned == "Democrat",])
fit6_s1_rep <- lm_robust(thermometer_3 ~ Z_fc_3, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$pid_1_cleaned == "Republican",])
fit6_s1_other <- lm_robust(thermometer_3 ~ Z_fc_3, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$pid_1_cleaned == "Independent/Other",])
```

```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(list(fit4_s1_dem, fit4_s1_rep, fit4_s1_other), include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact check"),
       custom.model.names = c("Democrats", "Republicans", "Other"),
       stars = c(0.05, 0.01, 0.001), 
       caption = "Fact-checking the claim that abortion pills are dangerous lowers Democrats' opinions of Alliance Defending Freedom (0-to-100 scale)", 
       caption.above = TRUE, float.pos = "H",
       single.row = TRUE)
```
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(list(fit5_s1_dem, fit5_s1_rep, fit5_s1_other), include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact check"),
       custom.model.names = c("Democrats", "Republicans", "Other"),
       stars = c(0.05, 0.01, 0.001), 
       caption = "Fact-checking the claim that contraception availability increases abortion demand lowers Democrats' opinions of Seth Gruber (0-to-100 scale)", 
       caption.above = TRUE, float.pos = "H",
       single.row = TRUE)
```
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(list(fit6_s1_dem, fit6_s1_rep, fit6_s1_other), include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact check"),
       custom.model.names = c("Democrats", "Republicans", "Other"),
       stars = c(0.05, 0.01, 0.001), 
       caption = "Fact-checking the claim about the pregnant 10-year-old has a statistically insignificant effect on Democrats' attitudes toward Americans United For Life (0-to-100 scale)", 
       caption.above = TRUE, float.pos = "H",
       single.row = TRUE)
```
...
```{r, echo = FALSE, results='asis'}
#Running models
fit4_s1_high_relig <- lm_robust(thermometer_1 ~ Z_fc_1, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$attend_cleaned == "High",])
fit4_s1_med_relig <- lm_robust(thermometer_1 ~ Z_fc_1, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$attend_cleaned == "Medium",])
fit4_s1_low_relig <- lm_robust(thermometer_1 ~ Z_fc_1, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$attend_cleaned == "Low",])

fit5_s1_high_relig <- lm_robust(thermometer_2 ~ Z_fc_2, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$attend_cleaned == "High",])
fit5_s1_med_relig <- lm_robust(thermometer_2 ~ Z_fc_2, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$attend_cleaned == "Medium",])
fit5_s1_low_relig <- lm_robust(thermometer_2 ~ Z_fc_2, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$attend_cleaned == "Low",])

fit6_s1_high_relig <- lm_robust(thermometer_3 ~ Z_fc_3, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$attend_cleaned == "High",])
fit6_s1_med_relig <- lm_robust(thermometer_3 ~ Z_fc_3, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$attend_cleaned == "Medium",])
fit6_s1_low_relig <- lm_robust(thermometer_3 ~ Z_fc_3, 
    data = abortion_s1_cleaned[abortion_s1_cleaned$attend_cleaned == "Low",])
```

```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(list(fit4_s1_low_relig, fit4_s1_med_relig, fit4_s1_high_relig), include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact check"),
       custom.model.names = c("Low religiosity", "Medium religiosity", "High religiosity"),
       stars = c(0.05, 0.01, 0.001), 
       caption = "Fact-checking the claim that abortion pills are dangerous reduces the low religiosity group's opinions of Alliance Defending Freedom (0-to-100 scale)", 
       caption.above = TRUE, float.pos = "H",
       single.row = TRUE)
```
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(list(fit5_s1_low_relig, fit5_s1_med_relig, fit5_s1_high_relig), include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact check"),
       custom.model.names = c("Low religiosity", "Medium religiosity", "High religiosity"),
       stars = c(0.05, 0.01, 0.001), 
       caption = "Fact-checking the claim that contraception availability increases abortion demand reduces the low religiosity group's opinions of Seth Gruber (0-to-100 scale)", 
       caption.above = TRUE, float.pos = "H",
       single.row = TRUE)
```
...
```{r, echo = FALSE, results='asis'}
#Creating regression table
texreg(list(fit6_s1_low_relig, fit6_s1_med_relig, fit6_s1_high_relig), include.ci = FALSE,
       custom.coef.names = c("Intercept", "Control", "Fact check"),
       custom.model.names = c("Low religiosity", "Medium religiosity", "High religiosity"),
       stars = c(0.05, 0.01, 0.001), 
       caption = "Fact-checking the claim about the pregnant 10-year-old has a statistically insignificant effect on the low religiosity group's attitudes toward Americans United For Life (0-to-100 scale)", 
       caption.above = TRUE, float.pos = "H",
       single.row = TRUE)
```
...
ma_combined_graph4_s1 <- ma_combined_2_s1
ma_combined_graph4_s1$var <- 1:3
ma_combined_graph4_s1$Effect <- "Fact check"
```
